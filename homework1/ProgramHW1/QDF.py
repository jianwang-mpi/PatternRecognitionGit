    ...
    with open("train-labels.idx1-ubyte", mode='rb') as trainLabelFile:
        with open("train-images.idx3-ubyte", mode="rb") as trainImageFile:
            ...
            for i in range(0, 10):
                ...
                temp = np.linalg.pinv(variance[i])
                w[i].shape = (28 * 28, 1)
                w[i] = temp.dot(mean[i])
                w0[i] = -0.5 * np.transpose(mean[i]).dot(temp).dot(mean[i])  # -0.5 * math.log(np.linalg.det(variance[i]+averageVariance))
...
def test():
    with open("t10k-images.idx3-ubyte", mode="rb") as testImageFile:
        with open("t10k-labels.idx1-ubyte", mode="rb") as testLabelFile:
            ...
            count_every = [0] * 10
            correct_every = [0] * 10
            tongji_every = [np.array([0]*10)]*10
            for i in range(0, number):
                max = float("-inf")
                for j in range(0, 10):
                    temp = w[j].dot(Images[i]) + w0[j] + np.transpose(Images[i]).dot(W[j]).dot(Images[i])
                    if temp > max:
                        max = temp
                        max_pos = j
                count_every[Labels[i]] += 1
                tongji_every[Labels[i]][max_pos]+=1
                if max_pos == Labels[i]:
                    count = count + 1
